    ...
    <div class="dp-main">
      <div class="dp-main-caption">
        ...
        <div v-for="(dateArr, idx) in dateArrs" :key="idx">
          <span class="dp-grid dp-grid-date" v-for="dateObj in dateArr" :key="dateObj.date" :class="dateObj.className">{{dateObj.dateDay}}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    execute (date) {
      ...
      for (let i = 0, oft = 0; i < rowAmount; i++) {
        Vue.set(this.dateArrs, i, [])
        for (let j = 0; j < 7; j++) {
          // this.dateArrs[i][j] = this.getDate(firstDay, oft)
          Vue.set(this.dateArrs[i], j, this.createDateObj(this.getDate(firstDay, oft), date))
          oft++
        }
      }
